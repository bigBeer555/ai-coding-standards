...
代码必须优先使用最直接、最少步骤的表达方式。禁止为了“看起来更安全”“看起来更清晰”而增加无业务价值的代码量。

禁止无意义的来回转换或中转赋值：

```ts
const nextExpanded = !this.tradeExpanded
this.tradeExpanded = nextExpanded
```

应直接表达状态变化：

```ts
this.tradeExpanded = !this.tradeExpanded
```
...
能够直接使用原数据渲染时，禁止重新包一层计算函数、格式函数或转发函数。
...
模板渲染优先直接使用已有字段。只有存在复杂表达式、跨字段组合、业务语义派生或复用价值时，才允许新增派生数据。
...
非 TypeScript 代码中，禁止因为类型不确定而防御性添加 `Number()`、`String()`、`Boolean()` 等基础类型方法。

允许例外：

- 存在明确的条件判断，用于匹配不同输入类型。
- `id` 等业务上确认不会为 `0` 的字段，可以将匹配判断降级为 `==`。
- 硬性需要数值转换时，优先使用 `+value`，禁止优先使用 `Number(value)`。

其他情况宁可不做防御，也不要在业务逻辑中补基础类型方法纠偏。
